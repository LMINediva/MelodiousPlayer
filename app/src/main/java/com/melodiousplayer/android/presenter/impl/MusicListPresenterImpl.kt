...
class MusicListPresenterImpl(var musicListView: MusicListView) : MusicListPresenter,
    ...
    override fun loadDatas() {
        MusicListRequest(MusicListPresenter.TYPE_INIT_OR_REFRESH, 0, this).execute()
    }

    override fun loadMore(offset: Int) {
        MusicListRequest(MusicListPresenter.TYPE_LOAD_MORE, offset, this).execute()
    }
    ...
    override fun onSuccess(type: Int, result: MusicListBean) {
        if (type == MusicListPresenter.TYPE_INIT_OR_REFRESH) {
            musicListView.loadSuccess(result)
        } else if (type == MusicListPresenter.TYPE_LOAD_MORE) {
