...
import com.melodiousplayer.android.net.UploadMVPosterRequest
import java.io.File

class UploadMVPosterPresenterImpl(val view: UploadPosterContract.View) :
    UploadPosterContract.Presenter, ResponseHandler<UploadFileResultBean> {

    override fun uploadPoster(token: String, file: File) {
        UploadMVPosterRequest(this).executePostUploadFile(token, "image/jpeg", file,
